...
function sendPost(div, postType, onSuccess) {
    if (logRequire()) {
        ...
        if (postType == "comment") {
            // 发送参数：评论
            if (!div.getAttribute("pid")) return newMsgBox("啊哦，nmFun 内部出现错误");
            inpost = div.getAttribute("pid");
            tocomment = div.getAttribute("rid");
            sendData = { type: "comment", pid: inpost, rid: tocomment, content: textToSend };
        }
        sendCover.setAttribute("open", "true");
        $("#sendCover p").html("正在发送");
        if (postType == "post") newAjax("POST", backEndURL + "/post/newpost.php", true, "", sendData, function (data) { $("[data-posts-num-uid=" + myUid + "]").html(data['post_num']); attachmentUpload(div, data['pid'], postType, function () { sendCover.setAttribute("open", "false"); onSuccess(); }); }, function (err) { sendCover.setAttribute("open", "false"); newMsgBox("抱歉，发送失败。<br>" + err['info']) });
        if (postType == "comment") newAjax("POST", backEndURL + "/comment/newcomment.php", true, "", sendData, function (data) { $("[data-comments-num-uid=" + myUid + "]").html(data['comment_num']); attachmentUpload(div, data['cid'], postType, function () { sendCover.setAttribute("open", "false"); onSuccess(); }); }, function (err) { sendCover.setAttribute("open", "false"); newMsgBox("抱歉，发送失败。<br>" + err['info']) });
    }
}
    ...
    for (i = 0; i < div.getElementsByClassName("mediasBox")[0].getElementsByClassName("m").length; i++) {
        ...
        try {
            ...
            if (mInfoParsed.type == "image" || mInfoParsed.type == "video") {
                ...
                console.log(file);
                newFileAjax("POST", backEndURL + "/attachment/attachment.php", true, `target_type=${postType}&action=add&type=${mInfoParsed.type}&${postType[0]}id=${pid}`, formData, function () { mediaUploadSta.push(true); verifyPostAttachmentUploadStatus(div, pid, postType, onSuc); }, function (data) { mediaUploadSta = false; verifyPostAttachmentUploadStatus(div, pid, postType, onSuc, onErr, data['info']); onSuc(); onErr(), "附件上传失败，因为" + data['info'] });
            }
            else if (mInfoParsed.type = "biliVideo") {
                newAjax("POST", backEndURL + "/attachment/attachment.php", true, `target_type=${postType}&action=add&type=bili&${postType[0]}id=${pid}&bvid=${mInfoParsed.bvid}&p=${mInfoParsed.p}`, undefined, function () { mediaUploadSta.push(true); verifyPostAttachmentUploadStatus(div, pid, postType, onSuc); }, function (data) { mediaUploadSta = false; verifyPostAttachmentUploadStatus(div, pid, postType, onSuc, onErr, "Bilibili 视频上传失败，因为" + data['info']);; onErr() });
            }
...
function verifyPostAttachmentUploadStatus(div, pid, postType, onSuc, onErr, msg = "") {
    if (mediaUploadSta == false) {
        deleteMyPost(postType, pid, "", false);
        $("#sendCover").attr("open", "false");
        newMsgBox("有附件上传失败，已为您删除" + (postType == "post" ? "帖子" : "评论") + "，请确认无误后重试。<br>若仍有问题，可联系 nmTeam 支持。<br>具体问题：" + msg);
        onErr();
    }
    ...
    if (mediaUploadSta.length == div.getElementsByClassName("mediasBox")[0].getElementsByClassName("m").length) {
        onSuc(pid);
        if (postType == "post") newPostDetailPage(pid);
    }
}
...
function comment(pid, rid = 0, userName) {
    showPostInput('comment', true);
    $("#commentEditBox").attr("pid", pid);
    $("#commentEditBox").attr("rid", rid);
    $("#commentEditBox .replyTip").attr("placeholder", "回复 " + userName + " 的" + (rid == 0 ? "帖子" : "评论"));
}
...
<div class="bottomBox">
    <div class="bottomSurface">
        ...
        <button onclick="changebottomBoxMoreStatus('{{id}}','formatDrawer');" title="格式"><i class="material-icons">text_format</i></button>
...
function formatInput(id, type) {
    try {
        htmlW = new Text("[" + type + "][/" + type + "]");
        newE = window.getSelection().focusNode.splitText(window.getSelection().focusOffset);
        window.getSelection().focusNode.parentNode.insertBefore(htmlW, newE);
    }
    catch (err) {
        console.log(err);
        $("#" + id + " .sendBoxInput").append(htmlW);
    }
}
